"""
ANDON BOARD SCRAPER v9 - dva cloud endpointy
Posila na Railway + PythonAnywhere zaroven.
"""
...
ANDON_BASE    = "http://hlczaps04:8080/ctt"
HEARTBEAT_SEC = 30

# ─── MAPOVANI SENZORU NA LINKY ────────────────────────────────────────────────

SENSOR_TO_LINE = {
    "a0110": "A01", "a0120": "A01", "a0130": "A01", "a0140": "A01",
    "a0150": "A01", "a0160": "A01", "a0170": "A01",
    "a0210": "A02", "a0220": "A02", "a0230": "A02", "a0240": "A02",
    "a0250": "A02", "a0260": "A02", "a0270": "A02",
...
def build_payload():
    alarm_lines = [ln for ln, s in line_states.items() if s["alarm"]]
    lines_payload = {}
    for ln, state in line_states.items():
        active = [
            {"sensor_id": sid, **info}
            for sid, info in state["sensors"].items()
            if info.get("value", 0) == 1.0
        ]
        lines_payload[ln] = {
            "alarm":         state["alarm"],
        ...
        for attempt in range(3):
            try:
                resp = requests.post(
                    ep["url"],
                    json=payload,
                    headers={"Content-Type": "application/json", "X-Secret": ep["secret"]},
                    timeout=5
                )
                resp.raise_for_status()
                alarms = payload["alarm_lines"]
                if alarms:
                    log.warning(f"[{ep['name']}] OK – PORUCHY: {', '.join(alarms)}")
                else:
                    log.info(f"[{ep['name']}] OK – vse v poradku")
                break
            except Exception as e:
                log.error(f"[{ep['name']}] Chyba (pokus {attempt+1}/3): {e}")
                time.sleep(1)
...
def process_message(params):
    ...
    sen_type = "teamleader" if "teamleader" in sensor_id else "comp"
    ...
    any_alarm  = any(s.get("value", 0) == 1.0 for s in line_states[line]["sensors"].values())
    ...
    if value == 1.0:
        ...
        log.warning(f"*** ALARM [{typ}]: {line} | {stroj}")
        cloud_queue.set()
    elif value == 0.0 and prev_alarm and not any_alarm:
        log.info(f"Alarm VYRESENY: {line}")
        cloud_queue.set()
...
def main():
    log.info("=" * 50)
    log.info("  ANDON SCRAPER v9")
    log.info("  Cloud endpointy:")
    for ep in CLOUD_ENDPOINTS:
        log.info(f"    - {ep['name']}: {ep['url']}")
    log.info(f"  Heartbeat: {HEARTBEAT_SEC}s | Linek: {len(ALL_LINES)}")
    log.info("=" * 50)
    ...
    while True:
        try:
            ws = websocket.WebSocketApp(
                "ws://hlczaps04:8080/ctt-rec/websocket",
                on_open=on_open,
                on_message=on_message,
                on_error=on_error,
                on_close=on_close,
                header={"Origin": "http://hlczaps04:8080"}
            )
